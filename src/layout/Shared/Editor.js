...
const Editor = ({ onContentChange, initialContent, templates, attributesList }) => {

  const attributes = isEmpty(attributesList) ? [
  ...
  return (
    <SunEditor
      ...
      setOptions={{
        ...
        plugins: {...plugins, UserAttributes: {
          name: 'UserAttributes',
          display: 'submenu',
          ...
          setSubmenu: function (core) {
              ...
              for (let i = 0, len = attributes.length, attr; i < len; i++) {
                  attr = attributes[i]
