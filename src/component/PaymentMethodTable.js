...
const PaymentMethodTable = () => {
    ...
    const deleteEntity = (entityID) => {
        navigate("modal-content", "delete-modal", "Payment Method", entityID, deletedRows, setDeletedRows)
    }

    return (
      <div className="add-user-container">
      <h1 className="add-user-header">All Payment Methods</h1>
        <table className="fl-table mt-5">
          ...
          <tbody>
            {paymentMethods?.map((row, index) => (
              ...
              <tr id={"payment-method" + "-" + row["id"]} key={index}>
                ...
                <td
                  style={{padding: "0"}}
                  onClick={() => navigate("modal-content", "show-modal", "PaymentMethod", row, ["id", ...columns])}
                  key="show" 
                  className="operation"
                >
                ...
                <td
                  style={{padding: "0"}}
                  key="edit" 
                  onClick={() => navigate("content", "edit-payment-method", row)} 
                  className="operation">
