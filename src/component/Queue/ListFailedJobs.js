...
const ListFailedJobs = ({ queue }) =>
{
    ...
    const { getQueueFailedJobs, searchQueueFailedJob } = useQueueApi()
    ...
    const setup = async (perPage) => {
        ...
        if (isEmpty(searchParams)) {
            opr = await getQueueFailedJobs(queue, perPage)
        }
        else {
            opr = await searchQueueFailedJob(queue, perPage, searchParams.column, searchParams.value)
        }
    ...
    const searchEntityColumn = async (column, value) => {
        const opr = await searchQueueFailedJob(queue, 10, column, value)
        if (opr) setJobs(opr)
    ...
    const onCheckShowAll = async () => {
        let opr = []
        if (isEmpty(searchParams)) {
            opr = await getQueueFailedJobs(queue, 1000000000000)
        } else {
            opr = await searchQueueFailedJob(queue, 1000000000000, searchParams.column, searchParams.value)
        }
        setJobs(opr)
    }

    const showJobHandler = (job) => {
        navigate("job-management", "show-failed-job", job)
    }
