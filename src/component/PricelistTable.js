...
const PricelistTable = () => {
    ...
    const deleteEntity = (entityID) => {
        navigate("modal-content", "delete-modal", "Pricelist", entityID, deletedRows, setDeletedRows)
    }

    return (
      <div className="add-user-container">
      <h1 className="add-user-header">All Pricelists</h1>
        <table className="fl-table mt-5">
          ...
          <tbody>
            {pricelists?.map((row, index) => (
              ...
              <tr id={"pricelist" + "-" + row["id"]} key={index}>
                ...
                <td
                  style={{padding: "0"}}
                  onClick={() => navigate("modal-content", "show-modal", "Pricelist", row, ["id", ...columns])}
                  key="show" 
                  className="operation"
                >
                ...
                <td
                  style={{padding: "0"}}
                  key="edit" 
                  onClick={() => navigate("content", "edit-pricelist", row)} 
                  className="operation">
