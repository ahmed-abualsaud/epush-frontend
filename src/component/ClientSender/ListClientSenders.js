...
const ListClientSenders = () =>
{
    ...
    const setup = async (perPage) => {
        ...
        if (isEmpty(searchParams)) {
            ...
            msg = await search("sender", criteria, perPage)
        } else {
            msg = await search(searchParams.enti, searchParams.crit, perPage)
        }
         
        setSenders(msg)
        setColumns(["name", "approved", "created_at"])
    }
    useEffect(() => {
    ...
    const searchEntityColumn = async (column, value) => {
        ...
        const msg = await search("sender", criteria, 10)
    ...
    const onCheckShowAll = async () => {
        let msg = []
        if (isEmpty(searchParams)) {
            msg = await search("sender", "user_id = " + user?.user?.id, 1000000000000)
        } else {
...
export default ListClientSenders