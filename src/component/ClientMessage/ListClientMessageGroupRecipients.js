...
const ListClientMessageGroupRecipients = ({messageGroup}) =>
{
    ...
    const setup = async (perPage) => {
        ...
        if (isEmpty(searchParams)) {
            ...
            msgrp = await search("message_group_recipient", criteria, perPage)
        } else {
    ...
    const onCheckShowAll = async () => {
        let msgrp = []
        if (isEmpty(searchParams)) {
            msgrp = await search("message_group_recipient", "user_id = " + user?.user?.id, 1000000000000)
        } else {
...
export default ListClientMessageGroupRecipients