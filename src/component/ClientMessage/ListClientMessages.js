...
const ListClientMessages = () =>
{
    ...
    const setup = async (perPage) => {
        ...
        if (isEmpty(searchParams)) {
            let criteria = "user_id = " + user?.user?.id + " AND approved = true"
        ...
        setMessages(msg)
        setColumns(["sender_name", "content", "notes", "total_cost", "number_of_segments", "number_of_recipients", "language", "sent_at"])
    }
    useEffect(() => {
    ...
    const searchEntityColumn = async (column, value) => {
        let criteria = "user_id = " + user?.user?.id + " AND approved = true And " + column + " LIKE '%" + value + "%'"
    ...
    const onSearch = async (criteria) => {
        criteria = "user_id = " + user?.user?.id + " AND approved = true And " + criteria
        const msg = await search("message", criteria, 10)
        if (msg) setMessages(msg)
        setSearchParams({entity: encodeString("message"), criteria: encodeString(criteria), enti: "message", crit: criteria})
    }
    ...
    const onCheckShowAll = async () => {
        let msg = []
        if (isEmpty(searchParams)) {
            msg = await search("message", "user_id = " + user?.user?.id + " AND approved = true", 1000000000000)
        } else {
    ...
    const addMessageHandler = () => {
        navigate("content", "add-client-message")
    }
...
export default ListClientMessages