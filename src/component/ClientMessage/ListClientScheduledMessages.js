...
const ListClientScheduledMessages = () =>
{
    ...
    const setup = async (perPage) => {
        ...
        if (isEmpty(searchParams)) {
            let criteria = "user_id = " + user?.user?.id + " AND approved = true AND scheduled_at >= '" + getDatetimeString() + "'"
        ...
        setMessages(msg)
        setColumns(["sender_name", "content", "notes", "total_cost", "number_of_segments", "number_of_recipients", "language", "scheduled_at", "created_at"])
    }
    useEffect(() => {
    ...
    const searchEntityColumn = async (column, value) => {
        let criteria = "user_id = " + user?.user?.id + " AND approved = true AND scheduled_at >= '" + getDatetimeString() + "' And " + column +  ((column === "approved") ? (" = " + (["true", "yes", "1"].includes(value) ? "true" : "false")) : (" LIKE '%" + value + "%'"))
    ...
    const onSearch = async (criteria) => {
        criteria = "user_id = " + user?.user?.id + " AND approved = true AND scheduled_at >= '" + getDatetimeString() + "' And " + criteria
        const msg = await search("message", criteria, 10)
        if (msg) setMessages(msg)
        setSearchParams({entity: encodeString("message"), criteria: encodeString(criteria), enti: "message", crit: criteria})
    }
    ...
    const onCheckShowAll = async () => {
        let msg = []
        if (isEmpty(searchParams)) {
            msg = await search("message", "user_id = " + user?.user?.id + " AND approved = true AND scheduled_at >= '" + getDatetimeString() + "'", 1000000000000)
        } else {
    ...
    const showMessageHandler = (message) => {
        navigate("content", "show-client-message", message)
    }
...
export default ListClientScheduledMessages