...
const ListClientUnapprovedMessages = () =>
{
    ...
    const setup = async (perPage) => {
        ...
        if (isEmpty(searchParams)) {
            let criteria = "user_id = " + user?.user?.id + " AND approved = false"
    ...
    const searchEntityColumn = async (column, value) => {
        let criteria = "user_id = " + user?.user?.id + " AND approved = false And " + column + " LIKE '%" + value + "%'"
    ...
    const onSearch = async (criteria) => {
        criteria = "user_id = " + user?.user?.id + " AND approved = false And " + criteria
        const msg = await search("message", criteria, 10)
        if (msg) setMessages(msg)
        setSearchParams({entity: encodeString("message"), criteria: encodeString(criteria), enti: "message", crit: criteria})
    }
    ...
    const onCheckShowAll = async () => {
        let msg = []
        if (isEmpty(searchParams)) {
            msg = await search("message", "user_id = " + user?.user?.id + " AND approved = false", 1000000000000)
        } else {
...
export default ListClientUnapprovedMessages