...
const AddPartner = () => {
    ...
    const addNewPartner = async () => {
        ...
        if (validate("add-user-form")) {
            ...
            user = await addPartner(user);
            if (! isEmpty(user)) {
                navigate("content", "list-partners")
                showAlert("Partner Added Successfully")
            } else {
                showAlert("Valid Partner Information Required")
            }
        }
    }
    ...
    return (
        <Page id="add-user-form" title="Add New Partner">
            ...
            <div className="button-container">
                <button className="button" onClick={() => addNewPartner()}>Add New Partner</button>
            </div>
        </Page>
    )
}
