...
const GetFolderFiles = ({ folder }) =>
{
    ...
    const setup = async (perPage) => {
        ...
        if (isEmpty(searchParams)) {
            let criteria = "folder_id = " + folder?.id
        ...
        setFiles(msg)
        setColumns(["company_name", "url", "created_at"])
    }
    useEffect(() => {
    ...
    const searchEntityColumn = async (column, value) => {
        let criteria = "folder_id = " + folder?.id + " AND " + column + " LIKE '%" + value + "%'"
    ...
    const onSearch = async (criteria) => {
        criteria = "folder_id = " + folder?.id + " AND " + criteria
        const msg = await search("file", criteria, 10)
        if (msg) setFiles(msg)
        setSearchParams({entity: encodeString("file"), criteria: encodeString(criteria), enti: "file", crit: criteria})
    }
    ...
    const onCheckShowAll = async () => {
        let msg = []
        if (isEmpty(searchParams)) {
            msg = await search("file", "folder_id = " + folder?.id , 1000000000000)
        } else {
    ...
    const addFileHandler = () => {
        navigate("content", "upload-file-to-folder", folder)
    }
    ...
    const deleteFileHandler = (file, onDelete) => {
        navigate("modal-content", "delete-file", file, onDelete)
    }
    ...
    return (
        ...
        <Page title={folder.name}>
            ...
            <Table>
                ...
                <TableBody>
                    <DataRows columns={columns} rows={files.data}>
                        {withOperationCellParameters(ShowRowCell, "showFunction", showFileHandler)}
                        {withOperationCellParameters(DeleteRowCell, "deleteFunction", deleteFileHandler)}
                    </DataRows>
                </TableBody>
            </Table>
